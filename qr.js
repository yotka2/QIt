...
if (selection_text.length < max_qr_chars) {
    ...
    var svgNode = QRCode({msg : selection_text});
    var viewBoxWidth = svgNode.viewBox.baseVal.width;
    var width = viewBoxWidth / 90 * 300;
    if (width > 300) {
        width = 300;
    }
    svgNode.style.width  = width + "px";
    svgNode.style.height  = width + "px";
    svgNode.style.display = "block";
    //selection_element.focusNode.parentElement.appendChild(svgNode)

    const addedDiv = document.createElement("div");
    addedDiv.appendChild(svgNode)
    addedDiv.style.backgroundColor = "#f0f8ff";
    addedDiv.style.border = "1px solid #000";
    addedDiv.style.padding = "5px";
    addedDiv.style.borderRadius = "10px";
    addedDiv.style.position = "absolute";
    addedDiv.style.zIndex = "9999";

    addedDiv.style.top = (rect.top + window.scrollY - addedDiv.offsetHeight - width - 40) + "px";
    addedDiv.style.left = rect.left - 15 + "px";
    ...
    addedDiv.appendChild(arrow);
    const arrow2 = document.createElement("div");
    arrow2.style.position = "absolute";
    arrow2.style.bottom = "-39px";
    arrow2.style.left = "15px";
    arrow2.style.border = "solid transparent";
    arrow2.style.borderTopColor = "#f0f8ff";
    arrow2.style.borderWidth = "20px";
    arrow2.style.zIndex = 1;
    addedDiv.appendChild(arrow2);
